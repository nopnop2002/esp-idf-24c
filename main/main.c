...
void app_main()
{
	ESP_LOGI(tag, "EEPROM is 24C%.02d",BITS);
	...
	i2c_master_driver_initialize(&dev, BITS, i2c_port, CONFIG_I2C_ADDRESS, CONFIG_SDA_GPIO, CONFIG_SCL_GPIO);
	uint16_t bytes = getMaxAddress(&dev);
